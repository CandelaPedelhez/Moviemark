...
const Newsletter = () => {
  ...
  function handleSubmit(e) {
    ...
    const found = movies.find(
      (element) => element.id === parseInt(input.id[0])
    );
    const foundtwo = movies.find(
      (element) => element.id === parseInt(input.id[1])
    );
    const foundthree = movies.find(
      (element) => element.id === parseInt(input.id[2])
    );
    console.log(found);
    dispatch(
      sendNewsletter({
        ...
        <div>
          <form className={styles.form} onSubmit={(e) => handleSubmit(e)}>
            ...
            <p className={styles.text}>
              Select three movies to send the newsletter
            </p>
            <div>
              <select
              ...
              >
                <optgroup label="Select Movies"></optgroup>
                {movies.map((tp) => {
                  return (
              ...
              {input.id?.map((t) => (
                <div className={styles.opt}>
                  <div>
                    {movies.map((el) => {
                      return el.id === parseInt(t) ? (
                        <p className={styles.titleopt}>{el.title}</p>
                      ) : (
                        <></>
                      );
                    })}
            ...
            {input.id.length === 3 ? (
              ...
              <button className={styles.bttndis} disabled type="submit">
                Submit
              </button>
            )}
          </form>
