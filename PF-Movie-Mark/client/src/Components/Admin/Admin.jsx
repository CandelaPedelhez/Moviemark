...
const Admin = () => {
  ...
  function SubmitAdmin() {
    if (userForAdmin.id !== "") {
      setError(false);
      setCharging(true);
      dispatch(makeOrQuitAdmin(userForAdmin)).then(() => {
        setCharging(false);
        setSuccess(true);
        refreshPage();
      });
  ...
  function SubmitQuitAdmin() {
    if (userForQuitAdmin.id !== "") {
      setErrorQuit(false);
      setCharging(true);
      dispatch(makeOrQuitAdmin(userForQuitAdmin)).then(() => {
        setSuccessQuit(true);
        setCharging(false);
        refreshPage();
      });
  ...
  function SubmitDelete() {
    if (userForDelete !== -1) {
      ...
      setCharging(true);
      dispatch(deleteUser(userForDelete)).then(() => {
        setCharging(false);
  ...
  function SubmitReset(e) {
    if (userForReset !== -1) {
      setErrorReset(false);
      setCharging(true);
      dispatch(revokePass(userForReset)).then(() => {
        setCharging(false);
      ...
      <div className={styles.page}>
        <div className={styles.account}>
          ...
          <div>
            ...
            <button className={styles.button} onClick={() => SubmitAdmin()}>
              Submit
            </button>
            {error === true ? (
              <p className={styles.error}>Please select one user</p>
            ) : (
              <></>
            )}
            {success === true ? (
          ...
          <div>
            ...
            <select
              className={styles.select}
              onChange={(e) => handleSelectQuitAdmin(e)}
            >
              <option value="" selected disabled hidden>
                Choose one user
              </option>
            ...
            <button className={styles.button} onClick={() => SubmitQuitAdmin()}>
              Submit
            </button>
            {errorQuit === true ? (
              <p className={styles.error}>Please select one user</p>
          ...
          <div>
            ...
            <select
              className={styles.select}
              onChange={(e) => handleSelectDelete(e)}
            >
              <option value="" selected disabled hidden>
                Choose one user
              </option>
            ...
            <button className={styles.button} onClick={() => SubmitDelete()}>
              Submit
            </button>
            {errorDelete === true ? (
              <p className={styles.error}>Please select one user</p>
          ...
          <div>
            ...
            <select
              className={styles.select}
              onChange={(e) => handleSelectReset(e)}
            >
              <option value="" selected disabled hidden>
                Choose one user
              </option>
            ...
            <button className={styles.button} onClick={() => SubmitReset()}>
              Submit
            </button>
            {errorReset === true ? (
              <p className={styles.error}>Please select one user</p>
